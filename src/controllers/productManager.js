...
export class ProductManager {
    ...
    async getProducts() {
        const prods = JSON.parse(await fs.readFile('./src/models/productos.json', 'utf-8'))
        return prods
    }
    ...
    async addProducts(prod) {
        const prods = JSON.parse(await fs.readFile('./src/models/productos.json', 'utf8'))
    ...
    async updateProduct(id, updatedFields) {
        ...
        if (prodIndex != -1) {
            ...
            await fs.writeFile('./src/models/productos.json', JSON.stringify(prods, null, 2))
        } else {
            return false
        }
    }
    ...
    async deleteProduct(id) {
        ...
        if (prodIndex != -1) {
            await fs.writeFile('./src/models/productos.json', JSON.stringify(prods.filter(producto => producto.id != id)))
        } else {
            return false
        }
    }
